...
- (void)prepareLayout {
    [super prepareLayout];
//    首先要获取collectionView当前section上的item的个数
...
//    会在当前的collectionView的内容
    CGFloat width = self.collectionView.frame.size.width - _sectionInset.left - _sectionInset.right;

//    计算一个间隙的宽度
    CGFloat iSapcing = floor(width - _columnCount *_itemWith)/(_columnCount - 1);
    
    _interiteSpacing = iSapcing;
